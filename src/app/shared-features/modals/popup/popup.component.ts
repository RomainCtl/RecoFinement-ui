...
export class PopupComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.item.movie_id) {
      this.movieService.getUserMeta(this.item.content_id).then((result: MovieMetaResponseDto) => {
        this.moviesUserData = result.content;
      });
    }
    if (this.item.serie_id) {
      this.seriesService.getUserMeta(this.item.content_id).then((result: SerieMetaResponseDto) => {
        this.seriesUserMeta = result.content;
      });
      this.seriesService.getEpisodes(this.item.content_id).then((result: EpisodeDtoResponse) => {
        this.episodes = result.content;
      });
    }
    if (this.item.game_id) {
      this.gameService.getUserMeta(this.item.content_id).then((result: GameMetaResponseDto) => {
        this.gameUserMeta = result.content;
      });
    }
    if (this.item.isbn) {
      this.bookService.getUserMeta(this.item.content_id).then((result: BookMetaResponseDto) => {
        this.bookUserMeta = result.content;
      });
    }
    if (this.item.app_id) {
      this.appService.getUserMeta(this.item.content_id).then((result: ApplicationMetaResponseDto) => {
        this.appUserMeta = result.content;
      });
    }
    if (this.item.track_id) {
      this.trackService.getUserMeta(this.item.content_id).then((result: TrackMetaResponseDto) => {
  ...
  saveRating(event: ClickEvent): void {
    if (this.item.track_id) {
      this.trackService.saveRating(this.item.content_id, { rating: event.rating });
    }
    if (this.item.game_id) {
      this.gameService.saveRating(this.item.content_id, { rating: event.rating });
    }
    if (this.item.isbn) {
      this.bookService.saveRating(this.item.content_id, { rating: event.rating });
    }
    if (this.item.app_id) {
      this.appService.saveRating(this.item.content_id, { rating: event.rating });
    }
    if (this.item.serie_id) {
      this.seriesService.saveRating(this.item.content_id, { rating: event.rating });
    }
    if (this.item.movie_id) {
      this.movieService.saveRating(this.item.content_id, { rating: event.rating });
    }
  }
  ...
  savePurchasedData(event: MatCheckboxChange): void {
    if (this.item.game_id) {
      this.gameService.savePurchasedState(this.item.content_id, { purchase: event.checked });
    }
    if (this.item.isbn) {
      this.bookService.savePurchasedState(this.item.content_id, { purchase: event.checked});
    }
    if (this.item.app_id) {
      this.appService.saveDownloadedState(this.item.content_id, { downloaded: event.checked});
    }
  }

  saveWatchedEpisodes(event: MatSliderChange): void {
    this.seriesService.saveWatchedEpisodes(this.item.content_id, { num_watched_episodes: event.value });
  }

  saveWatchedMovie(event: any): void {
    this.movieService.saveWatchedMovie(this.item.content_id, { saveWatchedMovies: event });
  }
